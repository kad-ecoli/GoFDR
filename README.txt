...
2.Set UniRef90 protein sequence database in "database/uniprot_sequence/uniref90.fasta"; (Because the size of the latest sequence database is very huge, we didn't include it in this source code package. You may download it from http://www.uniprot.org/downloads) and format the database by "./makeblastdb -in uniref90.fasta -parse_seqids -dbtype prot".
  A small subset of UniRef90 proteins from the "database/annotations/noiea_confirmed_iea_annotations.txt" list are available at "database/uniprot_sequence/uniref90.fasta" as a place holder.
